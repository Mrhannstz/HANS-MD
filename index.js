...
// Response dictionary for common phrases with emojis
...
// Read the environment variable from Heroku (default to "no" if not set)
const autoReactEnabled = process.env.AUTO_REACT === 'yes'; // If AUTO_REACT is "yes", auto-react is enabled

// Function to fetch response for a given keyword (only emojis)
function getResponse(keyword) {
  return responseDictionary[keyword.toLowerCase()] || ['🙋‍♂️', '🙋‍♀️']; // Fallback emojis if no match
}

// Example usage: Get response for "hello"
console.log(getResponse("hello"));

// Function to react automatically to messages based on keywords (only emojis)
async function autoReactToMessage(from, text) {
  if (!autoReactEnabled) {
    return; // Skip if auto-react is disabled
  }

  const response = getResponse(text);

  // Pick a random emoji from the response array
  const randomResponse = response[Math.floor(Math.random() * response.length)];

  // Send the emoji reaction (no text)
  await zokou.sendMessage(from, { text: randomResponse });
}

// Listen for new incoming messages (WhatsApp or other supported chat systems)
zokou.ev.on("message", async (messageData) => {
  const { from, text } = messageData;

  // React automatically to messages if enabled
  await autoReactToMessage(from, text);
});

// Optionally, you can add a generic "fallback" auto-reaction if no match is found
async function fallbackAutoReact(from) {
  await zokou.sendMessage(from, { text: "🤖" });
}

// Listen for other types of interactions or commands
zokou.ev.on("otherEvent", async (eventData) => {
  const { from, type } = eventData;

  // Handle other event types if needed (e.g., new contacts, added group members, etc.)
  if (type === "new_contact") {
    await zokou.sendMessage(from, { text: "🤖" });
  }

  // Fallback for unknown event types
  await fallbackAutoReact(from);
});
...
// List of over 100 "like" emojis and positive reactions
...
if (conf.AUTO_REACT_STATUS === "yes") {
  ...
  _0xf78a87.ev.on("messages.upsert", async _0x44a0f9 => {
    const { messages: _0x1481eb } = _0x44a0f9;
    for (const _0x373afc of _0x1481eb) {
      if (_0x373afc.key && _0x373afc.key.remoteJid === 'status@broadcast') {
        ...
        // Get a random emoji for reaction
